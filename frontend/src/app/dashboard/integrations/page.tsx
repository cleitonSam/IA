...
import {
  ...
  UserCheck, Search, Tag, Users, RefreshCw,
} from "lucide-react";
...
export default function IntegrationsPage() {
  ...
  const [frTestPhone, setFrTestPhone] = useState("");
  const [frTestResult, setFrTestResult] = useState<any>(null);

  // Etiquetas (Chatwoot labels) — listar e ver contatos
  ...
  const fetchLabels = () => {
    ...
    axios.get("/api-backend/management/chatwoot/labels", getToken())
      ...
      .catch(e => setLabelsErro(e?.response?.data?.detail || "Falha ao carregar labels"))
  ...
  useEffect(() => {
    if (activeTab !== "etiquetas" || isAdminMaster) return;
    fetchLabels();
  }, [activeTab, isAdminMaster]);
  ...
  const handleFranqueadaTest = async () => {
    ...
    try {
      ...
      setFrTestResult({ erro: e?.response?.data?.detail || "Falha na requisição" });
    } finally { setFrTesting(false); }
  };
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-white flex">
      ...
      <main className="flex-1 min-w-0 overflow-auto">
        ...
        <div className="relative z-10 p-8 lg:p-10 max-w-6xl mx-auto pb-20">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
            ...
            {!isAdminMaster && !isCustomTab(activeTab) && (
            ...
            {tabs.map(tab => {
              ...
              return (
                <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                  ...
                  {!isCustomTab(tab.id) && tabActive && (
                    <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse ml-1" />
                  )}
                  {!isCustomTab(tab.id) && tabConfigured && !tabIntegration?.ativo && (
          ...
          {isAdminMaster ? (
            ...
            <AnimatePresence mode="wait">
              ...
              {/* ── ETIQUETAS: lista as labels do Chatwoot ── */}
              {activeTab === "etiquetas" && (
                <motion.div key="etiquetas" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }}>
                  <div className="flex items-center justify-between mb-6">
                    <div>
                      <h3 className="text-xl font-black uppercase flex items-center gap-3">
                        <Tag className="w-5 h-5 text-amber-400" /> Etiquetas (Labels Chatwoot)
                      </h3>
                      <p className="text-xs text-slate-500 mt-1 max-w-2xl">
                        Todas as etiquetas configuradas na conta Chatwoot. Clique numa etiqueta pra ver os contatos vinculados.
                      </p>
                    </div>
                    <button onClick={fetchLabels} disabled={labelsLoading}
                      className="flex items-center gap-2 px-4 py-2 rounded-2xl bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white text-xs font-semibold transition-all disabled:opacity-50">
      ...
      {/* ── Modal: Contatos de uma etiqueta ── */}
      <AnimatePresence>
        {selectedLabel && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
