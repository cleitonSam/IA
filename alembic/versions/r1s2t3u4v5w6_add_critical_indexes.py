...
def upgrade() -> None:
    ...
    op.execute("""
        CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_conversas_empresa_phone
        ON conversas(empresa_id, phone)
    """)

    # Dashboard de conversas: ordenação por data de criação
    op.execute("""
        CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_conversas_empresa_created
    ...
    op.execute("""
        CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_metricas_empresa_data
        ON metricas_diarias(empresa_id, data DESC)
    """)


def downgrade() -> None:
    op.execute("DROP INDEX CONCURRENTLY IF EXISTS idx_conversas_empresa_phone")
    op.execute("DROP INDEX CONCURRENTLY IF EXISTS idx_conversas_empresa_created")
    op.execute("DROP INDEX CONCURRENTLY IF EXISTS idx_mensagens_conversa_created")
    op.execute("DROP INDEX CONCURRENTLY IF EXISTS idx_followups_pending")
    op.execute("DROP INDEX CONCURRENTLY IF EXISTS idx_metricas_empresa_data")
